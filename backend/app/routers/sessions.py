...
from app.schemas.sessions import SessionCreateRequest, SessionCreateResponse, SessionListResponse, SessionSingleResponse
...
async def list_sessions(
    ...
    session_list = [
        SessionSingleResponse(
            id=session.id, title=session.title, createdAt=session.created_at, updatedAt=session.updated_at
        )
        for session in sessions
    ]
    return success_response(
        message="会话列表查询成功", data=SessionListResponse(items=session_list, page=page, pageSize=page_size)
    )
